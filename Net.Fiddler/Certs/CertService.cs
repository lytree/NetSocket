...
namespace Net.Fiddler.Certs
{
    ...
    sealed class CertService
    {
        ...
        public bool CreateCaCertIfNotExists()
        {
            ...
            #else
            var privateKeyPem = this.caCert.GetRSAPrivateKey()?.ExportRSAPrivateKey();
           
            File.WriteAllText(this.CaKeyFilePath, new string(PemEncoding.Write("PRIVATE KEY",privateKeyPem)), Encoding.ASCII);
            File.WriteAllText(this.CaCerFilePath, new string(PemEncoding.Write("CERTIFICATE",caCert.GetRawCertData())), Encoding.ASCII);
            #endif
            return true;
        }
